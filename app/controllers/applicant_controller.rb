  ...
  def search
    ...
    if @college.nil?
      ...
      if !@college.nil?
        @applicant = []
        @college.each do |college|
          @applicant << Applicants.find_all_by_collegeId(college.id)
        end

        @applicant =Kaminari.paginate_array(Applicants.where('"Name" like ? ', "#{params[:search_name]}%")).page(params[:page]).per(20)
