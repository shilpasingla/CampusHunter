...
class ApplicantController < ApplicationController
  ...
  def search
    ...
    if params[:Final_Pursued]=="true"
      @applicant =Kaminari.paginate_array(Applicants.where('"Name" like ? AND college like ? AND Result = true ',"%#{params[:search_name]}%","#{params[:collegename]}")).page(params[:page]).per(20)
      else if params[:First_Tech_Pursued] == "true"
        @applicant =Kaminari.paginate_array(Applicants.where('"Name" like ? AND college like ? AND FirstStatus = true ',"%#{params[:search_name]}%","#{params[:collegename]}")).page(params[:page]).per(20)
          else if params[:Pairing_Pursued] == "true"
            @applicant =Kaminari.paginate_array(Applicants.where('"Name" like ? AND college like ? AND PairingStatus = true ',"%#{params[:search_name]}%","#{params[:collegename]}")).page(params[:page]).per(20)
